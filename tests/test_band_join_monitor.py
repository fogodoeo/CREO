...
class ChromeSelectionTests(unittest.TestCase):
    def test_render_environment_uses_persistent_headless_paths(self) -> None:
        ...
        with mock.patch.dict(
            os.environ,
            {
                ...
                "BAND_CHROME_USER_AGENT": "Test Chrome Agent",
            },
            clear=False,
        ):
        ...
        self.assertEqual(rendered["chrome_user_agent"], "Test Chrome Agent")
    ...
    def test_runtime_status_contains_counts_without_applicant_identity(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(payload["auth_session"]["cookie_count"], 0)
            self.assertEqual(
                payload["auth_session"]["user_agent_mode"],
                "browser-default",
            )
    ...
    def test_bootstrap_cookie_navigates_back_to_application_page(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertLess(
                methods.index("Network.setUserAgentOverride"),
                methods.index("Network.setCookie"),
            )
            self.assertLess(
...
class FollowUpQuestionTests(unittest.TestCase):
    ...
    def test_bootstrap_cookie_is_injected_without_persisting_secret(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config["diagnostic_file"] = str(Path(temp_dir) / "diagnostic.jsonl")
            config["band_start_url"] = (
                "https://www.band.us/band/101878670/applications"
            )
            with mock.patch.dict(
                os.environ,
            ...
            self.assertEqual(connection.cookies[0]["domain"], ".band.us")
            self.assertEqual(monitor._bootstrap_cookie_source, "header")

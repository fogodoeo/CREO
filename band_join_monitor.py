...
def apply_environment_overrides(config: Mapping[str, Any]) -> dict[str, Any]:
    ...
    string_overrides = {
        ...
        "BAND_CHROME_USER_AGENT": "chrome_user_agent",
...
class BandJoinMonitor:
    ...
    def _read_bootstrap_cookies(self) -> list[dict[str, Any]]:
        ...
        if serialized:
            ...
            if cookies:
                self._bootstrap_cookie_source = "json"
                return cookies

        header = os.environ.pop("BAND_COOKIE_HEADER", "").strip()
        if not header:
            return []
        self._bootstrap_cookie_source = "header"
        ignored = {
            "path",
            "domain",
            "expires",
    ...
    def _install_user_agent_override(self, connection: CDPConnection) -> None:
        configured = str(self.config.get("chrome_user_agent", "")).strip()
        user_agent = configured
        if not user_agent and bool(self.config.get("chrome_headless", False)):
            try:
                version = connection.call("Browser.getVersion", timeout=3)
            except Exception:
                version = {}
            browser_ua = str(version.get("userAgent", ""))
            if browser_ua:
                user_agent = browser_ua.replace("HeadlessChrome/", "Chrome/")
        if not user_agent:
            return
        try:
            connection.call(
            ...
            self.logger.info("Chrome User-Agent 적용 실패: %s", safe_for_log(exc))
            return
        self._user_agent_mode = "configured" if configured else "headless-masked"
        self.logger.info("Chrome User-Agent 모드: %s", self._user_agent_mode)
    ...
    def write_runtime_status(self) -> None:
        ...
        try:
            ...
            parsed_target = urllib.parse.urlsplit(configured_target)
            target_host = (parsed_target.hostname or "").lower()
            safe_target = ""
            if target_host == "band.us" or target_host.endswith(".band.us"):
                safe_target = urllib.parse.urlunsplit(
                    ("https", target_host, parsed_target.path or "/", "", "")
                )
            ...
            payload = {
                ...
                "auth_session": {
                    "cookie_count": self._bootstrap_cookie_count,
                    "installed_cookie_count": self._installed_cookie_count,
                    "cookie_source": self._bootstrap_cookie_source,
                    "user_agent_mode": self._user_agent_mode,
    ...
    def _enable_cdp(self, connection: CDPConnection) -> bool:
        ...
        self._install_user_agent_override(connection)
    ...
    def _periodic_session_check(
        ...
        if page_url:
            ...
            if hostname == "band.us" or hostname.endswith(".band.us"):
                self._session_page = f"{hostname}{parsed_page.path or '/'}"
            else:
                self._session_page = "external-redirect"

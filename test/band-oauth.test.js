...
function cookieValue(setCookie) {
    const match = String(setCookie || '').match(new RegExp(`^${STATE_COOKIE}=([^;]+)`));
...
test('signed session tokens reject tampering and expiry', () => {
    ...
    assert.throws(() => verifyToken(`${token.slice(0, -1)}x`, SESSION_SECRET, SESSION_TYPE, NOW), /invalid token/);
    assert.throws(() => verifyToken(token, SESSION_SECRET, SESSION_TYPE, NOW + 61_000), /expired token/);
});

test('OAuth start uses an exact callback URI and an HttpOnly state cookie', async () => {
    const oauth = createBandOAuth({ env: ENV, now: () => NOW });
    const rejected = new CapturedResponse();
    await oauth.handle(
        request('GET'),
        rejected,
        new URL('https://creok.example.com/api/band-oauth/start?return_url=https://evil.example/')
    );
    assert.equal(rejected.status, 400);

    const accepted = new CapturedResponse();
    await oauth.handle(request('GET'), accepted, new URL('https://creok.example.com/api/band-oauth/start'));
...
test('callback discards the BAND access token and creates a pseudonymous browser session', async () => {
    ...
    const fetchImpl = async (url, options) => {
        ...
        if (parsed.origin + parsed.pathname === 'https://auth.band.us/oauth2/token') {
            return jsonResponse({ access_token: 'raw-band-access-token' });
        }
    ...
    const oauth = createBandOAuth({ env: ENV, fetchImpl, now: () => NOW, logger: { error() {} } });

    const start = new CapturedResponse();
    await oauth.handle(request('GET'), start, new URL('https://creok.example.com/api/band-oauth/start'));
    const state = cookieValue(start.headers['Set-Cookie']);
    const callbackResponse = new CapturedResponse();
    await oauth.handle(
        request('GET', '', { cookie: `${STATE_COOKIE}=${encodeURIComponent(state)}` }),
        callbackResponse,
        new URL('https://creok.example.com/api/band-oauth/callback?code=one-time-code')
    );
    ...
    await oauth.handle(
        request('POST', JSON.stringify({ token: sessionToken }), { origin: 'https://survey.example.com' }),
        sessionResponse,

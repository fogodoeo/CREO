...
test('CREWARTS personality test uses minimal copy, Pretendard, and official share marks', () => {
    ...
    assert.match(html, /crewart-survey\.js\?v=20260815-axis-motion-v15/);
    ...
    assert.equal((script.match(/kicker: '(?:성향|강점과 주의점|궁합)'/g) || []).length, 3);
    ...
    assert.match(script, /class="cw-depth-keyword-label">핵심 키워드/);
    assert.match(script, /<h3>\$\{escapeHtml\(step\.title\)\}<\/h3>[\s\S]*<p>\$\{escapeHtml\(step\.body\)\}<\/p>[\s\S]*<footer>\$\{escapeHtml\(step\.note\)\}<\/footer>/);
    ...
    assert.match(script, /step\.locked \? `<button class="cw-depth-unlock"/);
    ...
    for (const scene of ['personality', 'strength', 'compatibility', 'axes', 'speed', 'house', 'locked']) {
    ...
    assert.match(html, /crewart-survey-core\.js\?v=20260815-axis-motion-v15/);
    assert.match(html, /crewart-survey-v4\.css\?v=20260815-axis-motion-v15/);

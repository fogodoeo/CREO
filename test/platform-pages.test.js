...
test('CREWARTS personality test uses minimal copy, Pretendard, and official share marks', () => {
    ...
    assert.match(html, /crewart-survey\.js\?v=20260815-mobile-pairs-v22/);
    ...
    assert.match(script, /data-depth-scrolly aria-label="결과 리포트 \$\{steps\.length\}단계"/);
    assert.equal((script.match(/kicker: '(?:나의 방식|궁합|성향 지표)'/g) || []).length, 3);
    ...
    assert.match(script, /step\.note \? `<footer>\$\{escapeHtml\(step\.note\)\}<\/footer>` : ''/);
    assert.match(script, /titleNode\.textContent = step\.railTitle/);
    assert.match(script, /const keywords = step\.keywords \|\| \[\]/);
    assert.doesNotMatch(script, /data-depth-body|data-depth-note|data-depth-unlock/);
    assert.match(script, /data-action="unlock-detail">회원 상세 보기/);
    assert.match(script, /new IntersectionObserver\(scheduleSync/);
    ...
    assert.match(css, /@media \(max-width: 760px\)[\s\S]*\.cw-depth-stage\s*\{[^}]*display:\s*none/);
    assert.match(script, /class="cw-depth-mobile-summary"/);
    assert.match(script, /class="cw-depth-mobile-summary-art"/);
    assert.match(script, /style="--depth-index:\$\{index\}"/);
    assert.match(css, /@media \(max-width: 760px\)[\s\S]*\.cw-depth-mobile-summary\s*\{[^}]*position:\s*sticky[^}]*top:\s*12px[^}]*display:\s*flex/);
    assert.match(css, /@media \(max-width: 760px\)[\s\S]*\.cw-depth-step > article\s*\{[^}]*position:\s*relative[^}]*margin-top:\s*28svh/);
    assert.doesNotMatch(script, /class="cw-depth-card-visual"/);
    ...
    assert.match(script, /railTitle: '자연스럽게 잘하는 것'/);
    ...
    assert.match(script, /--axis-position:\$\{axis\.position\}%/);
    assert.match(script, /--axis-fill-width:\$\{axis\.fillWidth\}%/);
    assert.match(css, /\.cw-depth-step\.is-active \.cw-depth-axis-track b\s*\{[^}]*animation:\s*cw-depth-axis-point/);
    assert.match(css, /\.cw-depth-step\.is-active \.cw-depth-axis-track::after\s*\{[^}]*animation:\s*cw-depth-axis-fill/);
    assert.match(css, /@keyframes cw-depth-axis-point\s*\{[\s\S]*left:\s*var\(--axis-position\)/);
    assert.match(css, /\.cw-depth-stage-art\s*\{[^}]*opacity:\s*1/);
    assert.doesNotMatch(css, /\.cw-depth-stage-art\s*\{[^}]*opacity:\s*\.94|\.cw-depth-stage-art\s*\{[^}]*opacity:\s*\.34/);
    assert.match(script, /visualNode\.classList\.add\('is-entering'\)/);
    assert.match(css, /\.cw-depth-stage-art img\.is-entering\s*\{[^}]*animation:\s*cw-depth-art-in/);
    assert.match(css, /\.cw-depth-step\.is-active > article\s*\{[^}]*transform:\s*translateY\(0\) scale\(1\)/);
    assert.match(css, /@keyframes cw-depth-copy-in/);
    assert.doesNotMatch(script, /facts:\s*axisFacts/);
    assert.match(script, /steps\.push\(\{[\s\S]*kicker: '성향 지표'[\s\S]*visual: 'axes'[\s\S]*\}\);[\s\S]*if \(!hasDetailedAccess\(\)\) return steps/);
    ...
    assert.match(html, /crewart-survey-core\.js\?v=20260815-mobile-pairs-v22/);
    assert.match(html, /crewart-survey-v4\.css\?v=20260815-mobile-pairs-v22/);
    ...
    assert.match(script, /class="cw-depth-outro"/);
    assert.doesNotMatch(script, /성향을 이해하기 위한 참고 결과입니다/);
    assert.match(script, /<small>© 2026 CREO\. All rights reserved\.<\/small>/);
    ...
    assert.match(script, /contentNode\.classList\.add\('is-entering'\)/);
    ...
    assert.match(script, /class="cw-depth-house"[\s\S]*당신의 기숙사는[\s\S]*\$\{escapeHtml\(house\.name\)\}/);

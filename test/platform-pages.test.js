...
test('CREWARTS personality test uses minimal copy, Pretendard, and official share marks', () => {
    ...
    assert.match(html, /crewart-survey\.js\?v=20260815-depth-scroll-v4/);
    assert.match(script, /function renderDeepResult\(options, profile, house\)/);
    assert.match(script, /new URLSearchParams\(window\.location\.search\)\.get\('report'\) === 'deep'/);
    assert.match(script, /url\.searchParams\.set\('report', 'deep'\)/);
    assert.match(script, /history\.pushState\(tabHistoryState\('result'\), document\.title, url\)/);
    assert.match(script, /if \(isV2\) \{\s*renderDeepResult\(options, profile, house\);\s*return;/);
    assert.match(script, /data-depth-scrolly aria-label="심층 결과 5단계"/);
    assert.equal((script.match(/kicker: '(?:성향|강점|주의점|궁합|가이드)'/g) || []).length, 5);
    assert.match(script, /function setupDeepScrollytelling\(steps\)/);
    ...
    assert.match(css, /\.cw-depth-stage\s*\{[^}]*position:\s*sticky[^}]*height:\s*min\(720px, calc\(100svh - 36px\)\)/);
    assert.match(css, /\.cw-depth-step\s*\{[^}]*min-height:\s*78svh/);
    ...
    assert.match(script, /classList\.toggle\('is-depth-view', isV2\)/);
    ...
    assert.match(html, /crewart-survey-core\.js\?v=20260815-depth-scroll-v4/);
    assert.match(html, /crewart-survey-v4\.css\?v=20260815-depth-scroll-v4/);
    ...
    assert.match(script, /\$\{copy\.title\}: \$\{copy\.left\}와 \$\{copy\.right\} 중 \$\{firstSelected \? copy\.left : copy\.right\} 쪽/);
    ...
    assert.doesNotMatch(script, /axisPoles|querySelectorAll\('\[data-pole\]'\)/);

...
test('BASIC page two reuses the one-line item and traits formula', () => {
    ...
    assert.match(source, /\.item-copy\.is-inline-info\{[^}]*width:max-content;max-width:min\(91\.2vw,1632px\)/);
    assert.match(source, /\.item-copy\.is-inline-info\[data-layout-custom="1"\]\{[^}]*right:calc\(100% - var\(--layout-x\) - var\(--layout-width\)\)!important[^}]*width:max-content!important;max-width:var\(--layout-content-width,var\(--layout-width\)\)!important/);
    assert.match(source, /slot==='p2-info'[^;]+--layout-content-width[^;]+Number\(placement\.width\)[^;]+\*1\.2/);
    assert.match(source, /slot==='p2-info'\|\|slot==='p2-progress'/);
    assert.match(source, /slot==='p2-progress'[^;]+--layout-content-height[^;]+Number\(placement\.height\)[^;]+\*1\.2/);
    assert.match(source, /data-layout-slot="p2-progress"[^}]*width:var\(--layout-content-width[^}]*height:var\(--layout-content-height/);
    ...
    assert.match(source, /body\[data-page="1"\] \.ticker-text,body\[data-page="2"\] \.ticker-text\{padding-left:var\(--ticker-after-badge-gap,72px\);font-size:var\(--ticker-text-font-size,24px\)\}/);
    assert.match(source, /function syncTickerGap\(element\)[^}]*selectNodeContents\(badge\)[^}]*--ticker-after-badge-gap/);
...
test('buyer and vendor checkout pages are short-code based and support the full payment workflow', () => {
    ...
    assert.match(vendor, /외부 알림톡 발송/);
    assert.match(vendor, /결제 안내 보냈어요/);
    assert.match(vendor, /링크 등록·알림 보내기/);
    assert.match(vendor, /실제 거래 내역 확인/);

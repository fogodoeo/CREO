...
test('CREWARTS uses one result journey and unlocks member detail by phone', () => {
    ...
    assert.match(html, /id="home-band-title">BAND로 계속하기/);
...
test('CREWARTS personality test uses minimal copy, Pretendard, and official share marks', () => {
    ...
    assert.match(html, /crewart-survey\.js\?v=20260815-clean-titles-v75/);
    ...
    assert.match(css, /\.cw-result-teaser-character img\s*\{[^}]*height:\s*clamp\(250px, 38svh, 360px\)[^}]*max-height:\s*clamp\(250px, 38svh, 360px\)/);
    ...
    assert.match(html, /crewart-survey-core\.js\?v=20260815-clean-titles-v75/);
    assert.match(html, /crewart-survey-v4\.css\?v=20260815-clean-titles-v75/);
    ...
    assert.match(script, /guestHouseKey[\s\S]*--guest-house-accent/);
    assert.match(css, /\.cw-result-teaser-hero\.is-guest-code > strong\s*\{[^}]*color:\s*var\(--guest-house-accent[^}]*font-size:\s*clamp\(104px, 28vw, 180px\)/);
    ...
    assert.match(script, /성향 분석 · 기숙사/);
    assert.match(script, /BAND 회원에게 전체 결과를 제공해요/);

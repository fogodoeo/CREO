(() => {
 ...
 for(const preset of CreoBroadcastPalette.presets){const button=document.createElement('button');button.type='button';button.dataset.palette=preset.id;button.setAttribute('aria-pressed','false');const dot=document.createElement('i');dot.style.background=preset.theme.secondary;dot.setAttribute('aria-hidden','true');button.append(dot,document.createTextNode(preset.name));button.onclick=()=>save(preset.theme);options.append(button);}
 function sync(){const found=CreoBroadcastPalette.match(current?.theme);$('theme-name').textContent=found?.name||'직접 지정';$('theme-color').value=current?.theme?.primary||'#315fbb';options.querySelectorAll('button').forEach(b=>{b.setAttribute('aria-pressed',String(found?.id===b.dataset.palette));b.disabled=busy||!current});$('theme-custom-apply').disabled=busy||!current;menu.dataset.busy=String(busy);}
 async function save(theme){
  if(busy||!current)return;const channelId=current.id;busy=true;$('theme-message').textContent='적용 중…';sync();
  try{
   // Send only the palette. Catalog CAS protects simultaneous channel-setting edits.
   const catalog=await CreoPlatform.api('channels');
   if(current.id!==channelId)throw new Error('채널이 바뀌었어요. 다시 선택해 주세요.');
   const result=await CreoPlatform.api(`channels/${encodeURIComponent(channelId)}`,{method:'PUT',body:JSON.stringify({channel:{theme},expectedVersion:catalog.version})});
   if(current.id!==channelId)return;
   current={...current,theme:result.channel.theme};
   $('control-frame').contentWindow?.postMessage({type:'creo-broadcast-theme',channelId,theme:current.theme},location.origin);
 ...
 $('theme-custom-apply').onclick=()=>save(CreoBroadcastPalette.custom($('theme-color').value));

...
function render(){
 ...
 const vendors=state.vendors.filter(v=>v.itemCount||v.history.length).sort((a,b)=>Number(b.remainingAmount>0)-Number(a.remainingAmount>0)||a.vendorName.localeCompare(b.vendorName,'ko'));
 $('vendors').innerHTML=vendors.map(v=>`<article class="settlement-card"><div class="settlement-pair"><b>${esc(v.vendorName)}</b><span class="settlement-status ${v.pendingReport?'waiting':v.remainingAmount?'':'done'}">${v.pendingReport?'입금 확인 요청':v.remainingAmount?'받아야 함':v.receivedAmount?'전액 받음':'받을 금액 없음'}</span></div><div class="settlement-vendor-amounts"><div><span>받을 총액</span><b>${money(v.totalAmount)}</b></div><div><span>받은 금액</span><b>${money(v.receivedAmount)}</b></div><div><span>미수금</span><b>${money(v.remainingAmount)}</b></div></div>${v.overpaidAmount?`<div class="settlement-pair"><span>초과 입금</span><b>${money(v.overpaidAmount)}</b></div>`:''}${v.history.length?`<details class="settlement-history"><summary class="settlement-muted">입금 내역 ${v.history.length}건</summary>${v.history.slice().reverse().map(r=>`<div class="settlement-history"><div class="settlement-pair"><span>${{pending:'확인 요청',confirmed:'입금 받음',rejected:'미입금'}[r.status]}</span><b>${money(r.amount)}</b></div><small class="settlement-muted">${date(r.reviewedAt||r.reportedAt)}</small></div>`).join('')}</details>`:''}</article>`).join('')||'<div class="settlement-empty">받을 배송비가 없어요</div>';

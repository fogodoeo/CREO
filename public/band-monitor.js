(() => {
    ...
    const sessionGuide = $('sessionGuide');
    let loading = false;
    ...
    const userAgentNames = {
        configured: '로컬값 적용',
        'headless-masked': '헤드리스 보정',
        'browser-default': '기본값'
    };
    const cookieSourceNames = { json: 'JSON · 도메인 보존', header: 'HEADER · 임시', none: '없음' };
    ...
    function render(data) {
        ...
        const authSession = monitor.auth_session || {};
        const cookieCount = Number(authSession.cookie_count) || 0;
        const installedCookieCount = Number(authSession.installed_cookie_count) || 0;
        $('cookieSourceValue').textContent = cookieSourceNames[authSession.cookie_source] || '확인 필요';
        $('cookieReadValue').textContent = cookieCount ? `${cookieCount}개` : '없음';
        $('cookieInstalledValue').textContent = installedCookieCount ? `${installedCookieCount}개` : '미적용';
        $('userAgentValue').textContent = userAgentNames[authSession.user_agent_mode] || '확인 필요';
        $('sessionPageValue').textContent = authSession.page || '—';
        if (authSession.target_url) {
            $('bandApplicationsLink').href = authSession.target_url;
        }
        const loginRequired = monitor.state === 'LOGIN_REQUIRED';
        sessionGuide.classList.toggle('needsAttention', loginRequired);
        $('sessionGuideLink').hidden = !loginRequired;
        setBadge(
            'sessionGuideBadge',
            bandReady ? '정상' : loginRequired ? '갱신 필요' : '대기',
            bandReady ? 'good' : loginRequired ? 'bad' : 'neutral'
        );

...
During live bidding P3 adds the active item's highest valid bid contribution to the settled total. This preview is never persisted as a sale. The live component becomes zero when the item sells, so the settled contribution replaces it without double counting. Team positions stay fixed while counters animate. The result panel background opacity is controlled independently with `page3ResultBackgroundOpacity` (0–100, default 85), preserving full-opacity text and logos.

...
export class Roulette extends EventTarget {
  ...
  private _updateMarbles(deltaTime: number) {
    ...
    for (let i = 0; i < this._marbles.length; i++) {
      ...
      if (marble.y > this._stage.goalY) {
        ...
        if (this._isRunning && this._winners.length === this._winnerRank + 1) {
          ...
          const remaining = this._marbles.find((candidate) => candidate !== marble && candidate.y <= this._stage!.goalY);
          if (remaining) {
            this.dispatchEvent(new CustomEvent('goal', { detail: { winner: remaining.name } }));
            this._winner = remaining;
            this._isRunning = false;
            this._particleManager.shot(this._renderer.width, this._renderer.height);
            setTimeout(() => {
              this._recorder.stop();
    ...
    if (
      ...
      this._marbles.forEach((m) => {
        ...
        this.physics.impact(m.id);
        this.physics.shakeMarble(m.id);
      });
  ...
  private _calcTimeScale(): number {
    ...
    if (this._winners.length < this._winnerRank + 1 && this._goalDist < zoomThreshold) {
      if (
        target.y > this._stage.zoomY - zoomThreshold * 1.2 &&
  ...
  public setTheme(theme: keyof typeof Themes | ColorTheme) {
    this._theme = typeof theme === 'string' ? Themes[theme] : theme;
  }
  ...
  public getStandings() {
    return [...this._winners, ...this._marbles].map((marble, index) => ({
      rank: index + 1,
      name: marble.name,
      finished: index < this._winners.length,
    }));
  }
